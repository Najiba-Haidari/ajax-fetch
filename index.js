...
axios.interceptors.response.use(
  ...
  (error) => {
    ...
    console.log(`Request took ${error.durationInMS} milliseconds.`);
    throw error;
  }
);
...
export async function favourite(imgId) {
  try {
    // Fetch the list of current favorites
    ...
    // console.log(response.data);
    // Check if the image with imgId is already
    const isFavorited = response.data.find(
      (favourite) => favourite.image_id === imgId
    );
    console.log(isFavorited ? true : false);
    if (isFavorited) {
      // unfavourite the image if it is favourited already
      // const existingFavorite = response.data.find(favourite => favourite.image_id === imgId);
      const favouriteData = await axios.delete(
        `https://api.thecatapi.com/v1/favourites/${isFavorited.id}`,
        {
          headers: {
            "x-api-key": API_KEY,
          },
      ...
      console.log(`Unfavorited image with image_id ${imgId}`);
    } else {
      // If not favorited, favorite the image / post to favourites, bodyRequest/data in axios is inside config
      const favData = await axios.post(
        "https://api.thecatapi.com/v1/favourites",
        {
          image_id: imgId,
        },
      ...
      console.log(`Favorited image with image_id ${imgId}`);
    }
  } catch (error) {
    console.error("Error while favoriting image:", error.message);
  }
}
...
async function getFavourites(event) {
  try {
    // const breedId = event.target.value;
    // console.log(breedId)
    const favouritesData = await axios.get(
      `https://api.thecatapi.com/v1/favourites`,
      {
        headers: {
          "x-api-key": API_KEY,
    ...
    favouritesData.data.forEach(async (favourite) => {
      if (favourite.image_id.length > 4) {
